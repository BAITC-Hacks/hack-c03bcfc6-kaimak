...
- **3D-город:** локальная Three.js r180, объёмные районы, кварталы с жилыми домами и стеклянными башнями, окна и балконы, дороги с разметкой и машинами, скверы, набережная, мосты с ограждениями и условный Байтерек с решётчатой опорой. Повторяющиеся детали объединены через InstancedMesh; в ночной теме окна подсвечены. Мышь или касание вращает модель; кнопки и щипок меняют масштаб. Стрелки, `+`/`−` и `Home` доступны с клавиатуры при фокусе на сцене. Подписи районов — обычные доступные кнопки. Выбор района также меняет цель для новых мер. Цветные объекты показывают выбранные мероприятия; переключатель «до / после» синхронизирован с расчётом.
